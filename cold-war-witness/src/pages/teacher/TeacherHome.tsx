  ...
  return (
    <Layout wide right={<TeacherMenu />}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="typewriter text-2xl font-bold">선생님 화면 · 내 학급</h1>
      ...
      {created && (
        <Notice tone="ok" className="mt-3">
          학급을 만들었어요. 학급 코드: <strong className="typewriter text-xl">{created}</strong> — 학생에게 칠판에 적어 알려 주세요.
        </Notice>
      )}
      {error && (
      ...
      <section className="mt-6" aria-label="학급 목록">
        {!classes ? (
          ...
          <p className="text-ink-soft">아직 만든 학급이 없어요. 위에서 학급을 만들어 주세요.</p>
        ) : (
          <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {classes.map((c) => (
              <li key={c.id}>
                <Link to={`/teacher/class/${c.id}`} className="dossier flex flex-col gap-1 p-4 hover:bg-white">
                  <span className="typewriter text-xl font-bold">📁 {c.name}</span>
                  <span>
                    학급 코드 <strong className="typewriter text-lg tracking-widest">{c.code}</strong>
                  </span>
