...
export default function FinalePage() {
  ...
  return (
    <Layout right={<StudentBadge />} wide>
      <h1 className="typewriter text-2xl font-bold">📜 나의 AI 윤리 실천 선언문</h1>
      ...
      <section className="dossier mt-5 p-4 sm:p-5" aria-label="원칙 카드 보드">
        ...
        <ul className="mt-3 grid gap-2 sm:grid-cols-3">
          {CORE_VALUES.map((v) => (
            <li key={v.id} className="rounded-md border-2 border-ink bg-white px-3 py-2 text-center">
            ...
            {[...owned, ...others].map((p) => {
              ...
              return (
                <button
                  ...
                  className={`min-h-11 rounded-full border-2 px-3 text-[16px] ${on ? 'font-bold text-white' : 'bg-white'}`}
                  style={{ borderColor: p.color, backgroundColor: on ? p.color : undefined }}
                >
                  {p.icon} {p.name}
                </button>
              );
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            {ERA_SUGGESTIONS.map((s) => (
              <button key={s} type="button" onClick={() => setEra(s)} className="min-h-11 rounded-full border border-line bg-white px-3 text-[16px] hover:bg-paper">
                {s}
              </button>
            ))}
          </div>
